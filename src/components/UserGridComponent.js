...
function UserGridComponent() {
  ...
  const fetchUserData = async () => {
    try {
      const response = await axios.get("http://localhost:8080/api/users"); // Assuming your backend endpoint is '/api/user'
  ...
  const columnDefs = [
    { headerName: "ID", field: "associateId" },
    { headerName: "NAME", field: "associateName" },
